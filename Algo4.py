...
#                 place_order(s,"USD","LIMIT","BUY",usd_qty,USD_bid+spread,max_qty,inventory,limit_orders,inventory_loss)
        ...
        if spread>=spread_limit[0]:
            ...
            if bull_qty>0:
                place_order(s,"BULL",order_type,"SELL",min(order_size/2,bull_qty),min(BULL_ask-spread,BULL_bid+0.01),max_qty,inventory,limit_orders,inventory_loss)
                #place_order(s,"CAD","LIMIT","BUY",bull_qty*(BULL_ask+spread),CAD_ask+spread,max_qty,inventory,limit_orders,inventory_loss)
            if bear_qty>0:
                place_order(s,"BEAR",order_type,"SELL",min(order_size/2,bear_qty),min(BEAR_ask-spread,BEAR_bid+0.01),max_qty,inventory,limit_orders,inventory_loss)
