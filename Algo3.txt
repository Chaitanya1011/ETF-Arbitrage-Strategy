...
def square_order(s,ticker,order_type,side,quantity,price,max_qty,limit_orders):
...
def place_order(s,ticker,order_type,side,quantity,price,max_qty,inventory,limit_orders,inventory_loss):
    ...
    if ticker=="USD" and (qty>=max_qty*20 or qty<=-1*max_qty*20):
        print("Too many USD")
        lose_inventory(s,ticker,qty,inventory_loss)
    else:
        if ticker=="USD":
...
def check_limit_orders(s,tick,limit_orders,max_holding_time,inventory):
    ...
    if len(limit_orders)>=1:
        ...
        for order_id in limit_orders:
            ...
            resp=s.get('http://localhost:9999/v1/orders', params={'id':order_id})
            #print(resp)
            if  resp.ok:            
                lmt_order = resp.json()    
                #print("hi")
                if len(lmt_order)>1:
                    #print(lmt_order)
                    if lmt_order[0]['status'] == "OPEN":
                        if (tick-limit_orders[order_id])>max_holding_time:
                            resp_cancel=s.delete('http://localhost:9999/v1/orders', params={'id':order_id})
                            if resp_cancel.ok:
                                limit_orders.pop(order_id, -1)
